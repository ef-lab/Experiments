...
class Camera():
    def __init__(self):
        ...
        self.camera.set_region (0,0,960,600)
        ...
        self.camera.set_exposure_time(90000)
    ...
    def capture(self):
        while not self.thread_end.is_set():
            ...
            if image:
                im = image.get_data()
                print(np.shape(im))
                dat = numpy.ndarray(buffer = im, dtype=self.dtype, shape=(960,600,1))
...
class Presenter(object):
    def __init__(self, shape=(960, 600), dtype=numpy.int8):
        self.queue = Queue()
        self.dtype = dtype
        self.shape = shape
        self.window=tk.Tk()
        ...
        self.window.geometry("%dx%d+%d+%d" % (900, 400, 630, 300))
        sv = tk.StringVar()
        sv.trace_add("write", lambda:print(2))
         # create the entries/buttons
        Entry3 = tk.Entry(self.window, textvariable=sv)
        Entry3.place(bordermode=tk.INSIDE, relheight=.08, relwidth=.40,relx=0.795,rely=0.8)
    ...
    def plot(self):    #Function to create the base plot, make sure to make global the lines, axes, canvas and any part that you would want to update later
        ...
        self.frame = self.ax.imshow(np.random.rand(960, 600),cmap='gray',vmin=1, vmax=2**8)
